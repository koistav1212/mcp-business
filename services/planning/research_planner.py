...
class DynamicResearchPlanner:
    ...
    def plan(self, plan_data: Any) -> List[Dict[str, str]]:
        ...
        if intent_clean == "sales_strategy" and not required_sources:
            return [
                {"task": "company_profile", "provider": "company"},
                {"task": "leadership", "provider": "people"},
                {"task": "hiring_signals", "provider": "people"},
                {"task": "competitor_mapping", "provider": "web"}
            ]
        ...
        if "company" not in required_sources:
            tasks.append({"task": "company_profile", "provider": "company"})
        ...
        if not tasks:
            tasks = [
                {"task": "company_profile", "provider": "company"},
                {"task": "recent_developments", "provider": "news"}
            ]
